...
fn test_retry_config() {
    ...
    let config = RetryConfig {
        max_retries: 5,
        backoff: BackoffStrategy::Constant(Duration::from_millis(500)),
        ..Default::default()
    };
...
async fn test_bulkhead_basic() {
    ...
    let config = BulkheadConfig {
        max_concurrent: 2,
        max_wait: Some(std::time::Duration::from_millis(100)),
    };

    let bulkhead = Bulkhead::new(config);

    // Should be able to acquire first permit
    let permit1 = bulkhead.acquire().await;
    assert!(permit1.is_ok());

    // Should be able to acquire second permit
